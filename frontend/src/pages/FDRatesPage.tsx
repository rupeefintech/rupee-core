...
import {
  ...
  Search, Star, ShieldCheck, RefreshCw, PiggyBank,
} from 'lucide-react';
...
function FreshBadge({ lastVerified }: { lastVerified: string }) {
  ...
  const cls  = days <= 7 ? 'bg-green-100 text-green-700' : days <= 30 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700';
        ...
        <div className="bg-gradient-to-br from-brand-800 via-brand-900 to-slate-900 text-white">
          <div className="max-w-5xl mx-auto px-4 py-12">
            <nav className="flex items-center gap-1.5 text-brand-300 text-xs mb-5">
              ...
              <span className="text-white font-medium">FD Interest Rates</span>
            </nav>
            ...
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-extrabold">Best FD Interest Rates 2026</h1>
                <p className="text-brand-200 mt-1 text-sm">Compare fixed deposit rates across all Indian banks</p>
              </div>
            </div>
            ...
            {!isLoading && !isError && (
              <div className="flex flex-wrap gap-4">
                {[
                  { label: 'Banks Tracked',     value: String(banks.length)        },
                  { label: 'Best Rate',          value: bestRate ? `${bestRate}%`   : '—' },
                  { label: 'Data Freshness',     value: 'Manually verified'          },
                ].map(s => (
        ...
        <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <select value={bankTypeFilter} onChange={e => setBankTypeFilter(e.target.value)}
                className="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-brand-400 bg-white">
              ...
              <label className="flex items-center gap-2 cursor-pointer px-3 py-2 border border-gray-200 rounded-xl hover:border-brand-300 transition-colors">
                ...
                <span className="text-sm text-gray-700 font-medium whitespace-nowrap">Senior Citizen Rates</span>
              </label>
          ...
          {!isLoading && !isError && banks.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-gray-900">Best FD Rates Right Now</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[...banks].sort((a, b) => b.bestRate - a.bestRate).slice(0, 4).map((b, idx) => {
                  const palette = [
                    { border: 'border-t-emerald-500', rate: 'text-emerald-700', badge: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
                    { border: 'border-t-brand-500',   rate: 'text-brand-700',   badge: 'bg-brand-50 text-brand-700 border-brand-200'       },
                    { border: 'border-t-blue-500',    rate: 'text-blue-700',    badge: 'bg-blue-50 text-blue-700 border-blue-200'          },
                    { border: 'border-t-violet-500',  rate: 'text-violet-700',  badge: 'bg-violet-50 text-violet-700 border-violet-200'    },
                  ];
                  const medal = ['🏆 Best Rate', '⭐ 2nd Best', '💎 Top Rated', '🔥 High Yield'];
                  const c = palette[idx];
                  return (
                    <div key={b.bank.id} className={`bg-white rounded-2xl shadow-sm border border-gray-100 border-t-4 ${c.border} p-5 flex flex-col gap-3 hover:shadow-md transition-shadow`}>
                      <div>
                        <span className={`text-[10px] font-bold px-2 py-0.5 border rounded-full ${c.badge}`}>{medal[idx]}</span>
                        <p className="text-[11px] text-gray-400 mt-2 font-semibold uppercase tracking-wide">{b.bank.bankType ?? 'Bank'}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          {b.bank.logoUrl && (
                            <img src={b.bank.logoUrl} alt={b.bank.name} className="w-5 h-5 object-contain rounded flex-shrink-0" />
                          )}
                          <p className="font-bold text-gray-900 text-sm leading-snug">{b.bank.name}</p>
                        </div>
                      </div>
                      <div>
                        <div className={`text-2xl font-extrabold ${c.rate}`}>{b.bestRate}% p.a.</div>
                        <div className="text-[11px] text-gray-400">{b.tenures.length} tenure{b.tenures.length !== 1 ? 's' : ''} available</div>
                      </div>
                      <button
          ...
          {isError && (
            <div className="bg-white rounded-2xl p-10 text-center shadow-sm border border-gray-100">
              <p className="text-gray-500 text-sm">Failed to load rates.{' '}
          ...
          {!isLoading && !isError && filtered.length === 0 && (
            <div className="bg-white rounded-2xl p-10 text-center shadow-sm border border-gray-100">
              <p className="text-gray-500 text-sm">No rates found. {banks.length === 0 ? 'Rates are being added — check back soon.' : 'Try clearing filters.'}</p>
            </div>
          )}
          ...
          {!isLoading && !isError && filtered.length > 0 && (
            <div className="space-y-3">
              {filtered.map((b, idx) => {
                ...
                return (
                  <div key={b.bank.id} className={`bg-white rounded-2xl shadow-sm border transition-all ${isExpanded ? 'border-brand-200' : 'border-gray-100 hover:border-gray-200'}`}>
                    {/* Bank header row */}
                    <button
                      onClick={() => toggleExpand(b.bank.id)}
                    ...
                    >
                      {/* Rank */}
                      <span className="w-7 h-7 rounded-full bg-brand-50 text-brand-700 font-bold text-xs flex items-center justify-center flex-shrink-0">
                        {idx + 1}
                      </span>
                      ...
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        {b.bank.logoUrl
                          ? <img src={b.bank.logoUrl} alt={b.bank.name} className="w-9 h-9 object-contain rounded-lg border border-gray-100 flex-shrink-0" />
                          : <div className="w-9 h-9 bg-brand-100 rounded-lg flex items-center justify-center text-brand-700 font-bold flex-shrink-0">{b.bank.name.charAt(0)}</div>}
                        <div className="min-w-0">
                          <div className="font-semibold text-gray-900 truncate">{b.bank.name}</div>
                      ...
                      <div className="hidden sm:flex items-center gap-1.5 flex-shrink-0">
                        <span className="text-xs text-gray-400">{b.tenures.length} tenure{b.tenures.length !== 1 ? 's' : ''}</span>
                        {isExpanded
                            ...
                            <tbody className="divide-y divide-gray-50">
                              {visibleTenures.map(t => (
                                <tr key={t.id} className="hover:bg-gray-50">
                                  <td className="px-5 py-3 font-medium text-gray-800">{t.label ?? '—'}</td>
                                  <td className="px-5 py-3 text-right font-bold text-emerald-700">{t.rate}%</td>
                                  ...
                                  <td className="px-5 py-3 text-right text-gray-500 hidden sm:table-cell text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-gray-900 text-sm">Need instant access?</p>
                <p className="text-xs text-gray-400 mt-0.5">Savings accounts up to 9% p.a. — no lock-in, withdraw anytime.</p>
              </div>
              <Link to="/savings-rates"
                className="flex items-center gap-1 px-3 py-2 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-900 transition-colors whitespace-nowrap text-xs flex-shrink-0">
            ...
            <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-gray-900 text-sm">Calculate FD maturity</p>
                <p className="text-xs text-gray-400 mt-0.5">Enter principal, rate & tenure — see exact returns with compounding.</p>
              </div>
              <Link to="/calculators/fd"
                className="flex items-center gap-1 px-3 py-2 bg-brand-800 text-white font-bold rounded-xl hover:bg-brand-900 transition-colors whitespace-nowrap text-xs flex-shrink-0">
          ...
          <p className="text-xs text-gray-400 text-center pb-4">
            ...
            <Link to="/savings-rates" className="text-brand-500 hover:underline">Need liquid savings? Compare savings rates →</Link>
          </p>
        </div>
      </div>
    </>
