  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-purple-50">
        <div className="bg-gradient-to-br from-purple-600 to-purple-800 text-white py-10 px-4 text-center">
          <h1 className="text-2xl font-bold mb-2">GST Calculator</h1>
          <p className="text-purple-100 text-sm max-w-md mx-auto">Calculate GST amount, CGST, SGST instantly for any product or service.</p>
        </div>

        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          <div className="bg-white rounded-lg shadow-lg border-l-4 border-purple-600 p-6">

            {/* Mode toggle */}
            <div className="flex gap-2 mb-6">
              {(['exclusive', 'inclusive'] as const).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={`px-5 py-2 rounded-full text-sm font-medium border transition-all ${mode === m ? 'bg-purple-600 border-purple-600 text-white' : 'border-slate-200 text-slate-500 hover:border-purple-300'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="number" value={amount} onChange={e => setAmount(Number(e.target.value))}
                      className="w-full pl-8 pr-4 py-3 border border-slate-200 rounded-lg text-slate-800 font-semibold focus:outline-none focus:border-purple-400 text-lg" />
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {GST_RATES.map(r => (
                      <button key={r} onClick={() => setGstRate(r)}
                        className={`py-2.5 rounded-lg text-sm font-bold border transition-all ${gstRate === r ? 'bg-purple-600 border-purple-600 text-white' : 'border-slate-200 text-slate-600 hover:border-purple-300 hover:text-purple-600'}`}>
                        {r}%
                      </button>
                    ))}
                  </div>
              ...
              {/* Result */}
              <div className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-lg p-5 text-white flex flex-col gap-3">
                <div>
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {GST_RATES.map(r => {
                      ...
                      return (
                        <tr key={r} className={`border-t border-slate-50 hover:bg-slate-50 ${r === gstRate ? 'bg-purple-50' : ''}`}>
                          <td className="px-4 py-3 font-bold text-purple-600">{r}%</td>
