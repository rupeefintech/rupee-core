    ...
    return (
      <>
        <div className="mt-4 h-10 rounded-xl overflow-hidden flex text-white text-sm font-bold">
          <div className="flex items-center justify-center bg-orange-400" style={{ width: `${iPct}%` }}>{iPct}%</div>
          <div className="flex items-center justify-center bg-blue-600 flex-1">{100 - iPct}%</div>
        </div>
        <div className="flex justify-between text-xs text-slate-400 mt-1">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-orange-400 inline-block"></span>Invested</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-600 inline-block"></span>Est. Returns</span>
        </div>
      </>
    );
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-green-50">
        <div className="bg-gradient-to-br from-green-600 to-green-800 text-white py-10 px-4 text-center">
          <h1 className="text-2xl font-bold mb-2">{meta.h1}</h1>
          <p className="text-green-100 text-sm max-w-md mx-auto">{meta.desc}</p>
        </div>

        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          <div className="bg-white rounded-lg shadow-lg border-l-4 border-green-600 p-6">

            {/* Tabs */}
            <div className="flex gap-1 bg-slate-100 p-1 rounded-xl mb-6 w-fit">
              {tabs.map(t => (
                <button key={t.key} onClick={() => setTab(t.key)}
                  className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${tab === t.key ? 'bg-green-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                  {t.label}
                </button>
              ))}
            </div>
            ...
            {tab === 'sip' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <SliderInput label="Monthly investment"         value={sipMonthly} min={500}   max={200000}  step={500}  display={fmtINR(sipMonthly)} onChange={setSipMonthly} parseInput={(raw: string) => Number(raw.replace(/[^0-9]/g, ''))} color="green" isZero={sipMonthly === 0} />
                  <SliderInput label="Expected return rate (p.a)" value={sipRate}    min={1}     max={30}      step={0.5}  display={`${sipRate}%`}       onChange={setSipRate}    color="green" isZero={sipRate === 0} />
                  <SliderInput label="Time period"                value={sipYears}   min={1}     max={40}      step={1}    display={`${sipYears} Yr`}    onChange={setSipYears}   color="green" isZero={sipYears === 0} />
                </div>
                <div className="flex flex-col justify-center">
                  ...
                  <button className="mt-4 bg-green-600 text-white font-bold text-sm rounded-xl py-3 hover:bg-green-700 transition-colors">
            ...
            {tab === 'lumpsum' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <SliderInput label="Total investment"           value={lsAmount} min={5000}  max={10000000} step={5000} display={fmtINR(lsAmount)} onChange={setLsAmount} parseInput={(raw: string) => Number(raw.replace(/[^0-9]/g, ''))} color="green" isZero={lsAmount === 0} />
                  <SliderInput label="Expected return rate (p.a)" value={lsRate}   min={1}     max={30}       step={0.5}  display={`${lsRate}%`}      onChange={setLsRate}   color="green" isZero={lsRate === 0} />
                  <SliderInput label="Time period"                value={lsYears}  min={1}     max={40}       step={1}    display={`${lsYears} Yr`}   onChange={setLsYears}  color="green" isZero={lsYears === 0} />
                </div>
                <div className="flex flex-col justify-center">
                  <div className="bg-slate-50 rounded-xl p-5 space-y-1">
            ...
            {tab === 'goal' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <SliderInput label="Target corpus"              value={goalAmount} min={100000} max={100000000} step={100000} display={fmtShort(goalAmount)} onChange={setGoalAmount} parseInput={(raw: string) => Number(raw.replace(/[^0-9]/g, ''))} color="green" isZero={goalAmount === 0} />
                  <SliderInput label="Expected return rate (p.a)" value={goalRate}   min={1}      max={30}        step={0.5}   display={`${goalRate}%`}        onChange={setGoalRate}   color="green" isZero={goalRate === 0} />
                  <SliderInput label="Time period"                value={goalYears}  min={1}      max={40}        step={1}     display={`${goalYears} Yr`}     onChange={setGoalYears}  color="green" isZero={goalYears === 0} />
                </div>
                <div className="flex flex-col justify-center">
                  <div className="bg-slate-50 rounded-xl p-5 space-y-1">
                    ...
                    {goalSIP > 0 && (
                      <div className="mt-4 bg-green-50 border border-green-100 rounded-xl p-3 text-center">
                        <div className="text-xs text-green-700 mb-1">You need to invest</div>
                        <div className="text-2xl font-bold text-green-700">{fmtINR(goalSIP)}<span className="text-sm font-normal">/month</span></div>
            ...
            {tab === 'sip' && (
              <>
                ...
                <div className="bg-green-50 border border-green-100 rounded-lg p-4 mb-5">
                  <p className="text-sm font-semibold text-green-800 mb-2">SIP Formula</p>
                  <code className="block bg-white rounded px-3 py-2 text-green-700 font-mono text-sm mb-2">M = P × [(1+r)ⁿ − 1] / r × (1+r)</code>
                  ...
                  {[
                    ...
                    <div key={col.title} className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                      ...
                      <ul className="space-y-1.5">
                        {col.items.map(item => (
                          <li key={item} className="text-xs text-slate-600 flex gap-2 items-start"><span className="text-green-500 flex-shrink-0 mt-0.5">✓</span>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ))}
                    ...
                    <tbody>
                      {[
                        ...
                        <tr key={i} className="border-t border-slate-50 hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-right font-bold text-green-600">{sip}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {[8, 10, 12, 14, 15, 18].map(r => {
                      ...
                      return (
                        <tr key={r} className={`border-t border-slate-50 hover:bg-slate-50 ${r === sipRate ? 'bg-green-50' : ''}`}>
                          <td className="px-4 py-3">
                            <span className={`font-bold ${r === sipRate ? 'text-green-700' : 'text-slate-700'}`}>{r}%</span>
                            {r === sipRate && <span className="ml-2 text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded">selected</span>}
                          </td>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex gap-3 text-sm items-start">
                    <span className="bg-green-600 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">{i + 1}</span>
          ...
          <div className="bg-green-50 border border-green-100 rounded-lg p-4 text-xs text-green-700 leading-relaxed">
