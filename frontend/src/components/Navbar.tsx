...
import {
  CreditCard, Wallet, Plane, Gift, Star, Globe, ArrowLeftRight, Calendar, TrendingUp, PiggyBank as PiggyBankIcon,
} from 'lucide-react';

const toolsMenu = [
  { label: 'FD Interest Rates',      path: '/fd-rates',            icon: TrendingUp,      desc: 'Best FD rates across all Indian banks'         },
  { label: 'Savings Account Rates',  path: '/savings-rates',       icon: PiggyBankIcon,   desc: 'Compare savings rates — zero balance & tiers'  },
  { label: 'SWIFT Code Lookup',      path: '/swift-code-lookup',   icon: Globe,           desc: 'Find SWIFT/BIC for international transfers'    },
  { label: 'Currency Converter',     path: '/currency-converter',  icon: ArrowLeftRight,  desc: 'Live INR ↔ USD, EUR, GBP, AED rates'           },
  { label: 'Bank Holidays',          path: '/bank-holidays',       icon: Calendar,        desc: '2025 & 2026 bank holiday calendar'             },
];
...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white/90 backdrop-blur-md border-b border-gray-100 sticky top-0 z-[999] shadow-sm">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <div className="relative" ref={toolsRef}>
              <button
                onClick={() => { setToolsOpen(!toolsOpen); setCalcOpen(false); setProdOpen(false); }}
                className={`flex items-center gap-1.5 ${linkCls(toolsOpen || location.pathname === '/swift-code-lookup' || location.pathname === '/currency-converter' || location.pathname === '/bank-holidays')}`}
              >
                Tools
              ...
              {toolsOpen && (
                <div className="absolute top-full right-0 mt-2 bg-white rounded-2xl shadow-xl border border-gray-100 p-3 z-[999]" style={{ width: '240px' }}>
                  <div className="flex flex-col gap-0.5">
                    {toolsMenu.map(item => {
                      const active = location.pathname === item.path;
                      return (
                        <Link key={item.path} to={item.path}
                          className={`flex items-start gap-3 px-3 py-2.5 rounded-xl transition-colors ${active ? 'bg-brand-50 text-brand-700' : 'hover:bg-gray-50 text-gray-700'}`}>
                          <item.icon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${active ? 'text-brand-500' : 'text-gray-400'}`} />
                          <div>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 flex flex-col gap-1 overflow-y-auto max-h-[calc(100vh-4rem)]">
          ...
          <button
            onClick={() => { setProdOpen(!prodOpen); setCalcOpen(false); setToolsOpen(false); }}
            className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600 text-left w-full"
          >
            <span>Products</span>
            <svg className={`w-3.5 h-3.5 transition-transform ${prodOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
          ...
          <button
            onClick={() => { setCalcOpen(!calcOpen); setProdOpen(false); setToolsOpen(false); }}
            className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600 text-left w-full"
          >
          ...
          {calcOpen && (
            <div className="pl-3 flex flex-col gap-3 py-2">
              ...
              <Link to="/calculators" className="px-2 py-1.5 text-sm font-semibold text-brand-600 hover:underline">View all calculators →</Link>
            </div>
          )}
          ...
          {/* Mobile Tools */}
          <button onClick={() => { setToolsOpen(!toolsOpen); setCalcOpen(false); setProdOpen(false); }}
            className="flex items-center justify-between px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600 text-left w-full">
          ...
          {toolsOpen && (
            <div className="pl-3 flex flex-col gap-1">
              {toolsMenu.map(item => (
                <Link key={item.path} to={item.path}
                  className="flex items-center gap-2 px-2 py-1.5 text-sm text-gray-600 hover:text-brand-600 rounded-lg hover:bg-brand-50">
                  <item.icon className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
                  {item.label}
                </Link>
              ))}
          ...
          <Link to="/loans" className="px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600">Loans</Link>
          <Link to="/accounts" className="px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600">Bank Accounts</Link>
          <Link to="/about" className="px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600">About</Link>
          <Link to="/contact" className="px-3 py-2 rounded-lg text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-600">Contact</Link>
