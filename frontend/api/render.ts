...
function renderIFSC(b: any): string {
  const ifsc      = b.ifsc || '';
...
function renderBank(data: any): string {
  const bankName  = data.bank || '';
  const total     = data.total_branches || 0;
  const branches  = (data.branches || []).slice(0, 20);
  const bankSlug_ = slug(bankName);
  const canonical = `${SITE}/bank/${bankSlug_}`;
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Real user: pass straight through to the React SPA
  if (!isBot) {
  ...
  // Bot: detect route, fetch data, return SSR HTML
  try {
    // /ifsc/:code
    ...
    if (ifscMatch) {
      ...
      if (res.ok) {
        ...
        return new Response(renderIFSC(data), {
          headers: {
    ...
    if (bankMatch) {
      const res = await fetch(`${BACKEND}/api/bank/${bankMatch[1]}`, {
        headers: { 'User-Agent': 'RupeePedia-Renderer/1.0' },
        signal:  AbortSignal.timeout(8000),
      });
      if (res.ok) {
        const data = await res.json();
        return new Response(renderBank(data), {
          headers: {
            'Content-Type':  'text/html; charset=utf-8',
            'Cache-Control': 's-maxage=86400, stale-while-revalidate=3600',
            'X-Renderer':    'bot-ssr',
    ...
    if (blogMatch) {
      ...
      if (res.ok) {
        const data = await res.json();
        return new Response(renderBlog(data), {
          headers: {
            'Content-Type':  'text/html; charset=utf-8',
    ...
    // Timeout or network error → fall through to SPA
  }
